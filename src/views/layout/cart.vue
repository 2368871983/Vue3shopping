<template>
  <div class="cart">
    <van-nav-bar left-arrow @click-left="$router.go(-1)" title="购物车" fixed />
    ...
    <div v-if="isLogin && cartList.length > 0">
        ...
        <div class="cart-item" v-for="item in cartList" :key="item.goods_id">
          ...
          <div class="info">
            ...
            <span class="bottom">
              ...
              <CountBox @input="(value) => changeCount(item.goods_id, value, item.goods_sku_id)"
      ...
      <div class="footer-fixed">
        <div class="all-check">
          <van-checkbox @click="handleAllCheck" :value="isAllCheck" icon-size="18"></van-checkbox>
          全选
        </div>
        ...
        <div class="all-total">
          ...
          <div v-if="!isEdit" @click="goPay" class="goPay" :class="{ disabled: selCount === 0 }">结算({{
            selCount }})</div>
...
export default {
  ...
  methods: {
    ...
    handleAllCheck () {
      this.$store.commit('cart/handleAllCheck', !this.isAllCheck)
    },
  ...
  watch: {
    isEdit (value) {
      this.$store.commit('cart/handleAllCheck', !value)
    }
  }
}
</script>
